...
unsigned char openFAT()
{
	...
	//Delays added for debug can remove.
...
try_readfat:
	...
	if( !IsFATOK )
	{
		...
		dumpSDDAT(0x164 + 8); //Jump to 0x1BE + 8 (we don't care about anything but the location info)
...
*/
		...
		dumpSDDAT( (lastfile->clusterno<<2) & 0x1ff );
